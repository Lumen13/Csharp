            ...
            for (int spec = 0; spec < 4; spec++)
            {
                ...
                if (spec == 3)
                {
                    while (true)
                    {
                        try
                        {
                            CheckType = Convert.ToDouble(Console.ReadLine());
                            cars.CarNewArray[spec] = Convert.ToString(CheckType);
                            break;
                        }
                        catch (Exception)
                        {
                            Console.WriteLine("\nЦена (в долларах) должна содержать только ");
                            Console.WriteLine("положительные числа без букв, символов и пробелов.");
                            Console.WriteLine("Введите цену автомобиля еще раз:\n");
                            continue;
                        }
                    }
                }
